...
public class SeqScan implements OpIterator {
	...
	public SeqScan(TransactionId tid, int tableid, String tableAlias) {
		...
		it = Database.getCatalog().getDatabaseFile(tableid).iterator(this.tid);
	}
	...
	public String getTableName() {
		// use table id to get the table name from catalog.
	...
	public TupleDesc getTupleDesc() {
		// some code goes here
		// gets the tuple desc with the renamed columns based 
		// on table aliases returns new tuple dest with new names
		...
		String[] renamedFields = new String[td.numFields()];
		for (int i = 0; i < td.numFields(); i++) {
			types[i] = td.getFieldType(i);
			String field = td.getFieldName(i) == null ? "null" : td.getFieldName(i);
			renamedFields[i] = this.tableAlias + "." + field;
		}
		return new TupleDesc(types, renamedFields);
	}
