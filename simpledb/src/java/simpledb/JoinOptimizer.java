...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if (joinOp == Predicate.Op.EQUALS) {
            if (t1pkey) {
                return card2; //first attribute is primary key
            } else if (t2pkey) {
                return card1; //second attribute is primary key
            } else if (card1 >= card2) {
                return card1; //simple heuristic added
            } else {
                return card2;
            }
        } else {
            return (int) (0.3*card1*card2); 
            //assuming 30% of the cross-product is emitted by range scans 
        }
    }
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        // returns a set of all of the subsets of joins of size 1
        for (int i = 0; i <= join.size(); i++) {
        	for (Set<LogicalJoinNode> set : enumerateSubsets(joins,i)) {
        		// Going through each join node and computing the cost and 
        		// card of subplan
                ...
                for (LogicalJoinNode s : set) {
                    CostCard plan = computeCostAndCardOfSubplan(stats, 
                    		filterSelectivities, s,set,best.cost,cache);
                    if (plan!=null && plan.cost < best.cost) {
                        best = plan;
                    }
                }
                // adding best plan to cache
